...
if ($#headers > 1 ){
	...
	$ibis = "/home/modupe/.bin/bin/ibis -d $chickengenes -q 'select genename, fpkm, library, chrom, chromstart, chromstop where library in \($specifics\) order by genename, chrom, chromstart' -v -o $output1.v1"; `$ibis`;
	$ibis = "/home/modupe/.bin/bin/ibis -d $mousegenes -q 'select genename, fpkm, library, chrom, chromstart, chromstop where library in \($specifics\) order by genename, chrom, chromstart' -v -o $output1.v2"; `$ibis`;
	$ibis = "/home/modupe/.bin/bin/ibis -d $alligatorgenes -q 'select genename, fpkm, library, chrom, chromstart, chromstop where library in \($specifics\) order by genename, chrom, chromstart' -v -o $output1.v3"; `$ibis`;
	print scalar(localtime),"\n"; 
	foreach my $filename (split(",", $filenamearray)){
		...
		while (<IN>){
			...
			if (exists $CHESS{"$genez|$chrom"}) { #if the key is already in the array.
				`date`;
				foreach my $fuzzyno (0..$#{$CHESS{"$genez|$chrom"}}) {
					my $fuzzy = ${$CHESS{"$genez|$chrom"}}[$fuzzyno];
					unless ($check eq "yes") {
