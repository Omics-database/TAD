...
DROP VIEW IF EXISTS `transcriptatlas`.`vw_libraryinfo` ;
DROP TABLE IF EXISTS `transcriptatlas`.`vw_libraryinfo`;
...
CREATE VIEW `vw_libraryinfo` AS select `a`.`sampleid` AS `sampleid`,`a`.`line` AS `line`,`a`.`species` AS `species`,`a`.`tissue` AS `tissue`,`a`.`notes` AS `notes`,`b`.`mapped_reads` AS `mapped_reads`,`b`.`genes` AS `genes`,`b`.`isoforms` AS `isoforms`,`c`.`totalvariants` AS `totalvariants`,`c`.`totalsnps` AS `totalsnps`,`c`.`totalindels` AS `totalindels` from ((`Sample` `a` join `MapStats` `b` on((`a`.`sampleid` = `b`.`sampleid`))) join `VarSummary` `c` on((`a`.`sampleid` = `c`.`sampleid`)));
