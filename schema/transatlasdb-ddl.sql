...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `GenesFpkm`;
CREATE TABLE `GenesFpkm` (`sampleid` VARCHAR(150) NOT NULL, `geneid` VARCHAR(100) NOT NULL DEFAULT '', `genename` VARCHAR(250) NOT NULL DEFAULT '', `chrom` VARCHAR(100) NOT NULL DEFAULT '', `start` INT(11) NOT NULL DEFAULT '0', `stop` INT(11) NOT NULL DEFAULT '0', `coverage` DOUBLE(20,10) NULL DEFAULT NULL, `tpm` DOUBLE(20,5) NULL DEFAULT NULL, `fpkm` DOUBLE(20,5) NOT NULL DEFAULT '0.00000', `fpkmconflow` DOUBLE(20,5) NULL DEFAULT NULL, `fpkmconfhigh` DOUBLE(20,5) NULL DEFAULT NULL, `fpkmstatus` VARCHAR(20) NULL DEFAULT NULL, INDEX `genesfpkm_indx_genename` (`genename` ASC), INDEX `genesfpkm_ibfk_1_idx` (`sampleid` ASC), PRIMARY KEY (`sampleid`, `geneid`, `genename`, `chrom`,`start`,`stop`,`fpkm`), KEY `genesfpkm_indx_genename_1` (`genename`), CONSTRAINT `genesfpkm_ibfk_1` FOREIGN KEY (`sampleid`) REFERENCES `GeneStats` (`sampleid`)) ENGINE = InnoDB DEFAULT CHARACTER SET = latin1;
-- -----------------------------------------------------
...
CREATE PROCEDURE `usp_gdtissue`(in gname varchar(45), in tissue varchar(45), in specie varchar(45)) select a.genename `Gene Name`, max(a.fpkm) `Maximum Fpkm`, CAST(avg(a.fpkm) AS DECIMAL(20,5)) `Average Fpkm`, min(a.fpkm) `Minimum Fpkm` from GenesFpkm a join vw_sampleinfo b on a.sampleid = b.sampleid where a.genename like CONCAT('%', TRIM(IFNULL(gname, '')), '%') and b.tissue = tissue and b.tissue is not null and b.organism = specie group by a.genename order by a.genename;
